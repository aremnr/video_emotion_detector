...
from PyQt5.QtWidgets import (
    ...
    QScrollArea, QSplitter, QSpacerItem, QTextEdit
)
...
def analyze_and_concatenate_video(csv_path, video_file):
    ...
    # === UPDATED LOGIC FOR FLAT STRUCTURE ===
    # Instead of creating a subfolder, we save directly into the project folder
    # where the source video exists.
    input_dir = os.path.dirname(os.path.abspath(video_file))
    
    # The output root is simply the directory containing the video
    output_root_folder = input_dir
    
    # We NO LONGER create "Processed_Output". 
    # Files go directly to output_root_folder.
    ...
    try:
        ...
        for category, segments in all_segments_df.items():
            ...
            # Create folder like "Project_5/Focus clips"
            ...
            if clips_for_montage:
                ...
                # Save montage directly to "Project_5/focus montage.mp4"
...
class VideoAnalyzerApp(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Pro Video Analyzer (No SDK)")
    ...
    def check_enable_button(self):
        ...
        if v:
            if self.load_existing_results(self.video_path_input.text(), self.csv_path_input.text()):
                self.populate_data(); self.analyze_button.setText("RE-PROCESS (Data Found)")
    ...
    def open_project_folder(self):
        ...
        if vid_path: 
            ...
            QMessageBox.information(self, "Info", "CSV found, but no video file found.")
    ...
    def on_success(self, res):
        ...
        if not res: return QMessageBox.info(self, "Info", "No highlights found based on thresholds.")
    ...
    def load_existing_results(self, v_path, c_path):
        ...
        # === UPDATED LOGIC FOR FLAT STRUCTURE ===
        # We look directly in the project folder (v_dir), not in Processed_Output
        out_dir = v_dir
        ...
        for cat in CATEGORY_CONFIG:
            ...
            # Check directly in the root folder
            ...
            if os.path.exists(m_path) and os.path.isdir(seg_dir):
                ...
                for i, f in enumerate(files):
                    try:
                        p = f[:-4].split('_')
                        # Expected format: segment_001_85.mp4
                        sid = int(p[1]); sc = float(p[2])
                        fpath = os.path.join(seg_dir, f)
                        
                        # Use moviepy to get duration only if strictly needed
